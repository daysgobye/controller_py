...
class Gamepad:
    def __init__(self, devices: List[object]) -> None:
        self._gamepad_device: Optional[object] = find_device(devices, usage_page=0x1, usage=0x05)

        self._report: bytearray = bytearray(6)
        self._last_report: bytearray = bytearray(6)

        self._buttons_state: int = 0
        self._joy_x: int = 0
        self._joy_y: int = 0
        self._joy_z: int = 0
        self._joy_r_z: int = 0
    ...
    def move_joysticks(self, x: Optional[int] = None, y: Optional[int] = None, z: Optional[int] = None, r_z: Optional[int] = None) -> None:
        if x is not None:
            self._joy_x = self._validate_joystick_value(x)
        if y is not None:
            self._joy_y = self._validate_joystick_value(y)
    ...
    @staticmethod
    def _validate_button_number(button: int) -> int:
        if not 1 <= button <= 16:
            raise ValueError("Button number must be in range 1 to 16")
        return button

    @staticmethod
    def _validate_joystick_value(value: int) -> int:
        if not -127 <= value <= 127:
            raise ValueError("Joystick value must be in range -127 to 127")
        return value
